...
class SemiCurrSinkhornKnopp(torch.nn.Module):
    ...
    """
    def __init__(self, num_iters=3, epsilon=0.1, gamma=1, stoperr=1e-6, numItermax=1000, rho=1, semi_use=True):
        super().__init__()
    ...
    def forward(self, P):
        ...
        for i in range(P.shape[0]):
            output.append(self.single_forward(P[i]))
...
if __name__=="__main__":
    logits = torch.zeros(262144, 16).cuda()
    print(logits)
    pp = SemiCurrSinkhornKnopp(rho=1).single_forward(logits)
    print(pp.sum(dim=0), pp.sum(dim=1))
    print(len(pp.sum(dim=0)), len(pp.sum(dim=1)))
    criterion = torch.nn.BCEWithLogitsLoss()
    print(criterion(logits, pp))